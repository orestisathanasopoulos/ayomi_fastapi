...
from app.schemas.operations import Operation, OperationWithResultInDb
...
def calculate(request: Operation, db: Session = Depends(get_db)):
    ...
    new_row = OperationWithResult(
        operation=request.operation, result=calculation)
    db.add(new_row)
    db.commit()
    db.refresh(new_row)
    return new_row
...
def fetchOperation(
    ...
    """
    results = db.execute(
        select(OperationWithResult).order_by(OperationWithResult.id))
